...
class TestGlobalTaggingV1():
    ...
    def clean_tags(self):
        ...
        tag_names = self.get_tag_names_for_resource(self, self.resource_crn,
                                                    'user')
        for tag_name in tag_names:
            if self.sdk_label in tag_name:
                print('Detaching user tag {0} from resource {1}'.format(
                    tag_name, self.resource_crn))
        ...
        for tag_name in tag_names:
            if self.sdk_label in tag_name:
                print('Detaching access tag {0} from resource {1}'.format(
                    tag_name, self.resource_crn))
        ...
        print('Found {0} user tag(s) that contain our label.'.format(
            len(tag_names)))
        for tag_name in tag_names:
        ...
        print('Found {0} access tag(s) that contain our label.'.format(
            len(tag_names)))
        for tag_name in tag_names:
    ...
    def delete_tag(self, tag_name, tag_type):
        response = self.global_tagging_service.delete_tag(tag_name=tag_name,
                                                          tag_type=tag_type)
    ...
    def get_tag_names_for_resource(self, resource_id, tag_type):
        ...
        response = self.global_tagging_service.list_tags(
            attached_to=resource_id, tag_type=tag_type)
    ...
    def list_tags_with_label(self, tag_type, label):
        ...
        while more_results:
            list_tags_response = self.global_tagging_service.list_tags(
                offset=offset, limit=500, tag_type=tag_type)
    ...
    def test_create_tag(self):
        create_tag_response = self.global_tagging_service.create_tag(
            tag_names=[self.access_tag_1, self.access_tag_2],
            tag_type='access')

        assert create_tag_response.get_status_code() == 200
        assert create_tag_response.get_result() is not None
        create_tag_results = CreateTagResults.from_dict(
            create_tag_response.get_result())
        assert create_tag_results is not None
        print('\ncreate_tag() result: ',
              json.dumps(create_tag_results.to_dict(), indent=2))
    ...
    def test_attach_tag_user(self):
        ...
        attach_tag_response = self.global_tagging_service.attach_tag(
            resources=[resource_model],
            tag_names=[self.user_tag_1, self.user_tag_2],
            tag_type='user')
        ...
        assert tag_results is not None
        print('\nattach_tag(user) result: ',
              json.dumps(tag_results.to_dict(), indent=2))
        ...
        tag_names = self.get_tag_names_for_resource(
            resource_id=self.resource_crn, tag_type='user')
    ...
    def test_attach_tag_access(self):
        ...
        # Make sure the tags are in fact attached to the resource.
        tag_names = self.get_tag_names_for_resource(
            resource_id=self.resource_crn, tag_type='access')
    ...
    def test_list_tags_user(self):
        ...
        while more_results:
            list_tags_response = self.global_tagging_service.list_tags(
                offset=offset, limit=500, tag_type='user')
            assert list_tags_response.get_status_code() == 200
            assert list_tags_response.get_result() is not None
        ...
        print('Found {0} user tags containing our label: {1}'.format(
            len(matches), matches))
    ...
    def test_list_tags_access(self):
        ...
        while more_results:
            list_tags_response = self.global_tagging_service.list_tags(
                offset=offset, limit=500, tag_type='access')
            assert list_tags_response.get_status_code() == 200
            assert list_tags_response.get_result() is not None
        ...
        for tag in tags:
            if self.sdk_label in tag.name:
                matches.append(tag.name)
        print('Found {0} access tags containing our label: {1}'.format(
            len(matches), matches))
    ...
    def test_detach_tag_access(self):
        ...
        # Make sure the tag is in fact attached to the resource
        tag_names = self.get_tag_names_for_resource(self.resource_crn,
                                                    'access')
    ...
    def test_delete_tag_user(self):
        delete_tag_response = self.global_tagging_service.delete_tag(
            tag_name=self.user_tag_1, tag_type='user')

        assert delete_tag_response.get_status_code() == 200
        assert delete_tag_response.get_result() is not None
        delete_tag_results = DeleteTagResults.from_dict(
            delete_tag_response.get_result())
        assert delete_tag_results is not None
        print('\ndelete_tag(user) result: ',
              json.dumps(delete_tag_results.to_dict(), indent=2))
    ...
    def test_delete_tag_access(self):
        delete_tag_response = self.global_tagging_service.delete_tag(
            tag_name=self.access_tag_1, tag_type='access')
    ...
    def test_delete_tag_all_user(self):
        ...
        delete_tags_result = DeleteTagsResult.from_dict(
            delete_tag_all_response.get_result())
        assert delete_tags_result is not None
        print('\ndelete_tag_all(user) result: ',
              json.dumps(delete_tags_result.to_dict(), indent=2))
