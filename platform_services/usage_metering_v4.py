...
class UsageMeteringV4(BaseService):
    ...
    def report_resource_usage(self, resource_id: str, resource_instance_usage: List['ResourceInstanceUsage'], **kwargs) -> DetailedResponse:
        """
        ...
        :param List[ResourceInstanceUsage] resource_instance_usage:
        ...
        if resource_instance_usage is None:
            raise ValueError('resource_instance_usage must be provided')
        resource_instance_usage = [ convert_model(x) for x in resource_instance_usage ]
        headers = {}
        ...
        data = json.dumps(resource_instance_usage)
        headers['content-type'] = 'application/json'

        if 'headers' in kwargs:
            headers.update(kwargs.get('headers'))

        url = '/v4/metering/resources/{0}/usage'.format(*self.encode_path_vars(resource_id))
    ...
    def report_cfresource_usage(self, resource_id: str, cf_resource_instance_usage: List['CfResourceInstanceUsage'], **kwargs) -> DetailedResponse:
        """
        Report Cloud Foundry resource usage.

        Report usage for resource instances that were provisioned by using the Cloud
        Foundry provisioning broker.

        :param str resource_id: The resource for which the usage is submitted.
        :param List[CfResourceInstanceUsage] cf_resource_instance_usage:
        :param dict headers: A `dict` containing the request headers
        :return: A `DetailedResponse` containing the result, headers and HTTP status code.
        :rtype: DetailedResponse with `dict` result representing a `ResponseAccepted` object
        """
        ...
        if resource_id is None:
            raise ValueError('resource_id must be provided')
        if cf_resource_instance_usage is None:
            raise ValueError('cf_resource_instance_usage must be provided')
        cf_resource_instance_usage = [ convert_model(x) for x in cf_resource_instance_usage ]
        headers = {}
        sdk_headers = get_sdk_headers(service_name=self.DEFAULT_SERVICE_NAME, service_version='V4', operation_id='report_cfresource_usage')
        headers.update(sdk_headers)

        data = json.dumps(cf_resource_instance_usage)
        headers['content-type'] = 'application/json'

        if 'headers' in kwargs:
            headers.update(kwargs.get('headers'))

        url = '/v1/metering/resources/{0}/usage'.format(*self.encode_path_vars(resource_id))
        request = self.prepare_request(method='POST',
                                       url=url,
                                       headers=headers,
                                       data=data)
...
class CfResourceInstanceUsage():
    """
    Usage information for a Cloud Foundry resource instance.

    :attr str organization_id: The ID of the orgnization to which the instance
          belongs. If the usage is for the instance is in a public cloud region, then the
          organization ID format is `{mccp_region_name}:{guid}`. If the usage is for a
          syndicated instance, then the organization ID format is
          `{mccp_region_id}:{guid}`.
    :attr str space_id: The ID of the space under which the instance is provisioned.
    :attr str resource_instance_id: The ID of the instance provisioned using the
          Cloud Foundry service controller.
    :attr str plan_id: The plan to meter the instance's usage with.
    :attr str region: The pricing region for the usage. For public regions, the
          region parameter value is the mccp region name in which the organization is
          present. For syndicated usage, use `us-south`.
    ...
    :attr str consumer_id: (optional) If an instance's usage should be aggregated at
          the consumer level, specify the ID of the consumer. Usage is accumulated to
          instance-consumer combination.
    """

    def __init__(self, organization_id: str, space_id: str, resource_instance_id: str, plan_id: str, region: str, start: int, end: int, measured_usage: List['MeasureAndQuantity'], *, consumer_id: str = None) -> None:
        """
        Initialize a CfResourceInstanceUsage object.

        :param str organization_id: The ID of the orgnization to which the instance
               belongs. If the usage is for the instance is in a public cloud region, then
               the organization ID format is `{mccp_region_name}:{guid}`. If the usage is
               for a syndicated instance, then the organization ID format is
               `{mccp_region_id}:{guid}`.
        :param str space_id: The ID of the space under which the instance is
               provisioned.
        :param str resource_instance_id: The ID of the instance provisioned using
               the Cloud Foundry service controller.
        :param str plan_id: The plan to meter the instance's usage with.
        :param str region: The pricing region for the usage. For public regions,
               the region parameter value is the mccp region name in which the
               organization is present. For syndicated usage, use `us-south`.
    ...
    @classmethod
    def from_dict(cls, _dict: Dict) -> 'CfResourceInstanceUsage':
        """Initialize a CfResourceInstanceUsage object from a json dictionary."""
        args = {}
        if 'organization_id' in _dict:
            args['organization_id'] = _dict.get('organization_id')
        else:
            raise ValueError('Required property \'organization_id\' not present in CfResourceInstanceUsage JSON')
        if 'space_id' in _dict:
            args['space_id'] = _dict.get('space_id')
        else:
            raise ValueError('Required property \'space_id\' not present in CfResourceInstanceUsage JSON')
        if 'resource_instance_id' in _dict:
            args['resource_instance_id'] = _dict.get('resource_instance_id')
        else:
            raise ValueError('Required property \'resource_instance_id\' not present in CfResourceInstanceUsage JSON')
        if 'plan_id' in _dict:
            args['plan_id'] = _dict.get('plan_id')
        else:
            raise ValueError('Required property \'plan_id\' not present in CfResourceInstanceUsage JSON')
        if 'region' in _dict:
            args['region'] = _dict.get('region')
        else:
            raise ValueError('Required property \'region\' not present in CfResourceInstanceUsage JSON')
        if 'start' in _dict:
            args['start'] = _dict.get('start')
        else:
            raise ValueError('Required property \'start\' not present in CfResourceInstanceUsage JSON')
        if 'end' in _dict:
            args['end'] = _dict.get('end')
        else:
            raise ValueError('Required property \'end\' not present in CfResourceInstanceUsage JSON')
        if 'measured_usage' in _dict:
            args['measured_usage'] = [MeasureAndQuantity.from_dict(x) for x in _dict.get('measured_usage')]
        else:
            raise ValueError('Required property \'measured_usage\' not present in CfResourceInstanceUsage JSON')
    ...
    def __str__(self) -> str:
        """Return a `str` version of this CfResourceInstanceUsage object."""
        return json.dumps(self.to_dict(), indent=2)

    def __eq__(self, other: 'CfResourceInstanceUsage') -> bool:
        """Return `true` when self and other are equal, false otherwise."""
        if not isinstance(other, self.__class__):
            return False
        return self.__dict__ == other.__dict__

    def __ne__(self, other: 'CfResourceInstanceUsage') -> bool:
...
class MeasureAndQuantity():
    ...
    def from_dict(cls, _dict: Dict) -> 'MeasureAndQuantity':
        ...
        if 'quantity' in _dict:
            args['quantity'] = object.from_dict(_dict.get('quantity'))
        else:
...
class ResourceInstanceUsage():
    ...
    def __init__(self, resource_instance_id: str, plan_id: str, start: int, end: int, measured_usage: List['MeasureAndQuantity'], *, region: str = None, consumer_id: str = None) -> None:
        """
...
class ResourceUsageDetails():
    ...
    :attr float status: A response code similar to HTTP status codes.
    ...
    def __init__(self, status: float, location: str, *, code: str = None, message: str = None) -> None:
        """
        Initialize a ResourceUsageDetails object.

        :param float status: A response code similar to HTTP status codes.
